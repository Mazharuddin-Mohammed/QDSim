...
// Constructor
SpinOrbitCoupling::SpinOrbitCoupling(const Mesh& mesh, const Material& material, SpinOrbitType type,
...
double SpinOrbitCoupling::calculate_rashba_parameter(const Material& material, double electric_field_z) {
...
double SpinOrbitCoupling::calculate_dresselhaus_parameter(const Material& material) {
...
void SpinOrbitCoupling::assemble_rashba_hamiltonian(Eigen::SparseMatrix<std::complex<double>>& H,
    ...
    // Get mesh data
    int n = mesh_.get_num_nodes();
    const auto& nodes = mesh_.get_nodes();
    const auto& elements = mesh_.get_elements();
