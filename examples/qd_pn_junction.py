#!/usr/bin/env python3
"""
Quantum Dot at P-N Junction Example
...
def create_realistic_config():
    ...
    config.N_A = 1e23  # Acceptor concentration (m^-3) - reduced for smaller depletion width
    config.N_D = 1e23  # Donor concentration (m^-3) - reduced for smaller depletion width
    ...
    def custom_potential(x, y):
        # P-N junction potential (depletion approximation)
        ...
        elif x < config.junction_position:
            # P-side depletion region
            V_pn = -config.e_charge * config.V_total * (x - (config.junction_position - config.W_p)) / config.W_p
        else:
            # N-side depletion region
            V_pn = -config.e_charge * config.V_total * (1 - (x - config.junction_position) / config.W_n)
...
def plot_separate_potentials(config):
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            ...
            elif X[i, j] < config.junction_position:
                # P-side depletion region
                V_pn[i, j] = -config.e_charge * config.V_total * (X[i, j] - (config.junction_position - config.W_p)) / config.W_p
            else:
                # N-side depletion region
                V_pn[i, j] = -config.e_charge * config.V_total * (1 - (X[i, j] - config.junction_position) / config.W_n)
    ...
    plt.tight_layout()
    plt.savefig('qd_pn_potentials.png', dpi=300, bbox_inches='tight')
...
def plot_1d_potential_slice(config):
    ...
    for i in range(len(x)):
        ...
        elif x[i] < config.junction_position:
            # P-side depletion region
            V_pn[i] = -config.e_charge * config.V_total * (x[i] - (config.junction_position - config.W_p)) / config.W_p
        else:
            # N-side depletion region
            V_pn[i] = -config.e_charge * config.V_total * (1 - (x[i] - config.junction_position) / config.W_n)
    ...
    # Save figure
    plt.savefig('qd_pn_potential_slice.png', dpi=300, bbox_inches='tight')
...
def main():
    ...
    # Save the figure
    plt.savefig('qd_pn_results.png', dpi=300, bbox_inches='tight')
