#!/usr/bin/env python3
"""
C++ Self-Consistent P-N Junction Example
...
using the C++ self-consistent Poisson solver.
...
def main():
    ...
    # Create the SimpleSelfConsistentSolver (faster but less accurate)
    print("Creating C++ SimpleSelfConsistentSolver...")
    solver = qdc.create_simple_self_consistent_solver(
        mesh, epsilon_r_callback, rho_callback
    )

    # SimpleSelfConsistentSolver doesn't have convergence acceleration parameters
    ...
    ax4.plot(x, E_field_x[mid_y, :], 'b-', linewidth=2)
    ax4.set_xlabel('x (nm)')
    ...
    plt.tight_layout()
    plt.savefig('cpp_self_consistent_pn_junction.png', dpi=300, bbox_inches='tight')
    plt.show()
