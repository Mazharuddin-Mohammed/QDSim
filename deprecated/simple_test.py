#!/usr/bin/env python3
"""
Simple test script for the QDSim simulator.

This script creates a very simple quantum dot simulation and generates
...
def main():
    ...
    # Domain size (smaller for faster computation)
    config.Lx = 100e-9  # 100 nm
    config.Ly = 50e-9   # 50 nm
    
    # Mesh parameters (coarser for faster computation)
    config.nx = 51  # Number of elements in x direction
    config.ny = 26  # Number of elements in y direction
    ...
    # Solve for the first 3 eigenvalues
    print("\nSolving for eigenvalues...")
    eigenvalues, eigenvectors = sim.solve(3)
...
def plot_results(sim, eigenvalues, eigenvectors):
    ...
    plt.tight_layout()
    plt.savefig('simple_test_results.png', dpi=300)
    print("\nResults saved to simple_test_results.png")
